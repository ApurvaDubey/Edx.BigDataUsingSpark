...
groupedByDay =dayToHostPairTuple.groupByKey().map(lambda x:(x[0], list(x[1])))
...
badUniqueEndpoints = badEndpoints.reduceByKey(lambda a,b:a+b).sortBy(lambda x:-1*x[1])
...
badEndpointsTop20 = badRecords.map(lambda x:(x.endpoint,1)).reduceByKey(lambda a,b:a+b).sortBy(lambda x:-1*x[1]).take(20)
...
errHostsTop25 = badRecords.map(lambda x:(x.host,1)).reduceByKey(lambda a,b:a+b).sortBy(lambda x:-1*x[1]).take(25)
...
topErrDate = errDateSorted.sortBy(lambda x:x[1]*-1).take(5)
