...
def simpleTokenize(string):
    ...
    """
    text = string
    #L = re.split(',',re.sub('\s+','',re.sub('[!"#$%&\'()*+,-./:;<=>?@[\\]^`{|}~ ]',',',re.sub('\s*$','',re.sub('^\s*','',text.lower())))))

    L = re.split(r'\W+',text.lower().strip())
...
def tokenize(string):
    ...
    return [x for x in L if len(x)>0 if x not in stopwords]
...
#print amazonRecToToken.take(2)

#amazonRecToToken.take(5)
def countTokens(vendorRDD):
    ...
    """
    tkn_cnt = vendorRDD.map(lambda x:len(x[1])).reduce(lambda x,y:x+y)
...
def findBiggestRecord(vendorRDD):
    ...
    return vendorRDD.takeOrdered(1,lambda x:len(x[1])*-1)
...
def tf(tokens):
    ...
    for i in set(tokens):
        ...
        for (j,k) in enumerate(tokens):
            if i==tokens[j]: cnt=cnt+1
        d[i]=cnt/float(len(tokens))
...
def idfs(corpus):
    ...
    idf = (corpus
                .map(lambda x:list(set([(i,1) for i in x[1]])))
                .flatMap(lambda x:x)
                .reduceByKey(lambda x,y:float(x+y))
...
def tfidf(tokens, idfs):
    ...
    tfIdfDict = {k1:v1*v2 for (k1,v1) in idfs.items() for (k2,v2) in tfs.items() if k1==k2}
    
    return tfIdfDict #tfIdfDict
...
def dotprod(a, b):
    ...
    """
    return sum([v1*v2 for (k1,v1) in a.items() for (k2,v2) in b.items() if k1==k2])
...
def norm(a):
    ...
    """
    return math.sqrt(sum([v1*v1 for (k1,v1) in a.items()]))
