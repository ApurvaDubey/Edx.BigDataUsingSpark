...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    movie_cnt=len(IDandRatingsTuple[1])
    rtg_avg=sum(IDandRatingsTuple[1])/float(movie_cnt)
    return (movie,(movie_cnt,rtg_avg))
...
movieIDsWithRatingsRDD = ratingsRDD.map(lambda x:(list(x)[1],list(x)[2])).groupByKey() 
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda x:(x[0],(len(x[1]),float(sum(x[1]))/len(x[1])) ))
...
movieNameWithAvgRatingsRDD = moviesRDD.join(movieIDsWithAvgRatingsRDD).map(lambda x:(list(x)[1][1][1], list(x)[1][0],list(x)[1][1][0]))
